...
const { PublishTaskEvent, publishMessage } = require("../utils");
const { TASK_BIND_KEY } = require("../config");

module.exports = {
    addTask: async function (req, channel) {
        ...
        try {
            if (taskData) {
                ...
                publishMessage(channel, TASK_BIND_KEY, JSON.stringify(data))
    ...
    removeTask: async function (req, channel) {
        req.body.userId = req.user.id
    ...
    updateTask: async function (req, channel) {
        req.body.userId = req.user.id
